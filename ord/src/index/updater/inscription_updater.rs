...
impl<'a, 'db, 'tx> InscriptionUpdater<'a, 'db, 'tx> {
  pub(super) fn new(
    ...
    satpoint_to_id: &'a mut MultimapTable<
      ...
      &'static InscriptionIdValue,
    >,
    satpoint_to_id_record: &'a mut MultimapTable<
      'db,
      'tx,
      &'static SatPointValue,
      &'static InscriptionIdValue,
  ...
  pub(super) fn index_envelopes(
    ...
    for (input_index, tx_in) in tx.input.iter().enumerate() {
      ...
      while let Some(inscription) = envelopes.peek() {
        ...
        if inscription.payload.media() == Media::Text {
          INDEX_CACHE
            .text_inscription_ids
            .write()
            .insert(inscription_id, ());
  ...
  fn update_inscription_location(
    ...
    let (outpoint, satpoint) = if unbound {
      let new_unbound_satpoint = SatPoint {
        outpoint: unbound_outpoint(),
        offset: self.unbound_inscriptions,
      };
      self.unbound_inscriptions += 1;
      (new_unbound_satpoint.outpoint, new_unbound_satpoint.store())
    } else {
      (new_satpoint.outpoint, new_satpoint.store())
    };
    ...
    self
      .satpoint_to_id_record
      .insert(&satpoint, &inscription_id)?;

    let mut index_cache_guard = INDEX_CACHE.outpoint_to_inscription_ids.write();
    match index_cache_guard.get_mut(&outpoint) {
      None => {
        index_cache_guard.insert(outpoint, HashMap::from([(flotsam.inscription_id, ())]));
      }
      Some(list) => {
        list.entry(flotsam.inscription_id).or_insert_with(|| ());
      }
    };
