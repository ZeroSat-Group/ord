...
pub(super) type BlockHashAndTxidValue = [u8; 64];

impl Entry for BlockHashWithTxid {
  type Value = BlockHashAndTxidValue;

  fn load(value: Self::Value) -> Self {
    let (blockhash, txid) = value.split_at(32);
    Self {
      block_hash: BlockHash::from_raw_hash(Hash::from_byte_array(blockhash.try_into().unwrap())),
      txid: Txid::from_raw_hash(Hash::from_slice(txid).unwrap()),
    }
  }
  ...
  fn store(self) -> Self::Value {
    let mut value = [0; 64];
    let (blockhash, txid) = value.split_at_mut(32);
    blockhash.copy_from_slice(self.block_hash.as_ref());
    txid.copy_from_slice(self.txid.as_ref());
    value
  }
}
