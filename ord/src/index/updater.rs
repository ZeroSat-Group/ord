...
// const BACKUP_DURATION_SEC: i64 = 60 * 60 * 24;
// const BACKUP_SAVE_DAYS: i64 = 5;
lazy_static! {
  static ref LAST_UPDATE_AT: parking_lot::Mutex<Option<DateTime<Local>>> =
    parking_lot::Mutex::new(Some(Local::now()));
}
...
impl<'index> Updater<'_> {
  ...
  pub(crate) fn update_index(&mut self) -> Result {
    ...
    if uncommitted > 0 {
      self.commit(wtx, value_cache)?;
      log::debug!("uncommitted committed")
    }

    if let Some(progress_bar) = &mut progress_bar {
      progress_bar.finish_and_clear();
      log::debug!("finish_and_clear done")
    }
  ...
  fn index_block(
    ...
    let mut block_hash_to_height = wtx.open_table(BLOCK_HASH_TO_HEIGHT)?;
    let mut blockhash_txid_to_txindex = wtx.open_table(BLOCKHASH_AND_TXID_TO_TXINDEX)?;
    let mut satpoint_to_inscription_id_record =
      wtx.open_multimap_table(SATPOINT_TO_INSCRIPTION_ID_RECORD)?;
    let mut outpoint_to_address = wtx.open_table(OUTPOINT_TO_ADDRESS)?;
    let mut txid_to_output_num = wtx.open_table(TXID_TO_OUTPUT_NUM)?;
    ...
    let mut inscription_updater = InscriptionUpdater::new(
      ...
      &mut satpoint_to_inscription_id_record,
      block.header.time,
    ...
    if self.height >= index.first_inscription_height {
      for (tx_offset, (_, txid)) in block.txdata.iter().enumerate() {
        let bhtxi = BlockHashWithTxid {
          block_hash: block.header.block_hash(),
          txid: *txid,
        };
        blockhash_txid_to_txindex.insert(&bhtxi.store(), u32::try_from(tx_offset)?)?;
      }

      block_hash_to_height.insert(&block.header.block_hash().store(), &self.height)?;
    }
  ...
  fn index_transaction_sats(
    ...
    outpoint_to_address: &mut Table<&OutPointValue, &[u8]>,
    txid_to_output_num: &mut Table<&TxidValue, u32>,
    first_inscription_height: u64,
  ) -> Result {
    if index_inscriptions {
    ...
    if self.height >= first_inscription_height {
      txid_to_output_num.insert(&tx.txid().store(), tx.output.len() as u32)?;
    }
    ...
    for (vout, output) in tx.output.iter().enumerate() {
      ...
      if self.height >= first_inscription_height {
        outpoint_to_address.insert(&outpoint.store(), output.script_pubkey.clone().as_bytes())?;
      }
