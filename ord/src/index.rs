...
use {
  self::{
    entry::{
      BlockHashAndTxidValue, BlockHashValue, Entry, InscriptionEntry, InscriptionEntryValue,
...
lazy_static! {
  pub static ref INDEX_CACHE: Arc<Cache> = Arc::new(Cache::default());
}

#[derive(Default)]
pub struct Cache {
  pub text_txids: RwLock<HashMap<Txid, ()>>,
  pub text_inscription_ids: RwLock<HashMap<InscriptionId, ()>>,
  pub outpoint_to_inscription_ids: RwLock<HashMap<OutPoint, HashMap<InscriptionId, ()>>>,
}
...
define_table! {BLOCKHASH_AND_TXID_TO_TXINDEX, &BlockHashAndTxidValue, u32}
define_table! { BLOCK_HASH_TO_HEIGHT, &BlockHashValue, u64 }
define_multimap_table! { SATPOINT_TO_INSCRIPTION_ID_RECORD, &SatPointValue, &InscriptionIdValue }
define_table! {OUTPOINT_TO_ADDRESS, &OutPointValue, &[u8]}
define_table! {TXID_TO_OUTPUT_NUM, &TxidValue, u32}

#[derive(Debug, Clone, PartialEq)]
pub(crate) struct BlockHashWithTxid {
  block_hash: BlockHash,
  txid: Txid,
}

impl BlockHashWithTxid {
  pub(crate) fn new(block_hash: BlockHash, txid: Txid) -> Self {
    Self { block_hash, txid }
  }
}
...
impl Index {
  pub(crate) fn cache_height(&self, height: Height) {
    if height.0 >= self.first_inscription_height {
      log::info!("caching height {}", height);
      let rtx = match self.rtx() {
        Ok(rtx) => rtx,
        Err(e) => {
          log::error!("caching height {} failed with error: {}", height, e);
          return;
        }
      };

      match self.get_txids_by_height(height, &rtx) {
        Ok(txids) => {
          if let Ok(sums) = self.get_txid_output_num(&txids, &rtx) {
            txids.into_iter().enumerate().for_each(|(idx, txid)| {
              (0..sums[idx]).into_iter().for_each(|n| {
                let outpoint = OutPoint { txid, vout: n };
                match self.get_inscriptions_on_output_record(outpoint, &rtx) {
                  Ok(inscription_ids) => {
                    if !INDEX_CACHE
                      .outpoint_to_inscription_ids
                      .read()
                      .contains_key(&outpoint)
                    {
                      INDEX_CACHE
                        .outpoint_to_inscription_ids
                        .write()
                        .insert(outpoint, inscription_ids.clone());
                    } else {
                      let mut guard = INDEX_CACHE.outpoint_to_inscription_ids.write();
                      if let Some(list) = guard.get_mut(&outpoint) {
                        inscription_ids.iter().for_each(|id| {
                          if !list.contains_key(id.0) {
                            list.insert(id.0.clone(), ());
                          }
                        })
                      }
                      drop(guard);
                    }
                    inscription_ids.into_iter().for_each(|id| {
                      if let Ok(Some(inscription)) = self.get_inscription_by_id_with_rtx(id.0, &rtx)
                      {
                        if inscription.media() == Media::Text {
                          INDEX_CACHE.text_inscription_ids.write().insert(id.0, ());
                          INDEX_CACHE.text_txids.write().insert(txid, ());
                        }
                      }
                    })
                  }
      ...
      log::info!("caching height {} done", height);
    }
  }

  pub(crate) fn rtx(&self) -> Result<ReadTransaction> {
    self.database.begin_read().map_err(Error::from)
  }

  pub(crate) fn get_raw_transaction_info(&self, txid: Txid) -> Result<GetRawTransactionResult> {
    Ok(self.client.get_raw_transaction_info(&txid, None)?)
  }
  ...
  pub(crate) fn get_inscriptions_on_output_record(
    &self,
    outpoint: OutPoint,
    rtx: &ReadTransaction,
  ) -> Result<HashMap<InscriptionId, ()>> {
    if let Some(inscriptions) = INDEX_CACHE
      .outpoint_to_inscription_ids
      .read()
      .get(&outpoint)
    {
      Ok(inscriptions.clone())
    } else {
      Ok(
        Self::inscriptions_on_output_unordered(
          &rtx.open_multimap_table(SATPOINT_TO_INSCRIPTION_ID_RECORD)?,
          outpoint,
        )?
  ...
  pub(crate) fn inscriptions_count(&self) -> Result<u64> {
    self
      .database
      .begin_read()?
  ...
  pub(crate) fn get_inscription_by_id_with_rtx(
    &self,
    inscription_id: InscriptionId,
    rtx: &ReadTransaction,
  ...
  #[allow(dead_code)]
  pub(crate) fn get_txids_by_height(
    &self,
    height: Height,
    rtx: &ReadTransaction,
  ) -> Result<Vec<Txid>> {
    if let Some(block_hash) = self.get_block_hash_by_height(height, Some(rtx))? {
      Ok(self.client.get_block_info(&block_hash)?.tx)
    } else {
      Ok(Default::default())
    }
  }
  ...
  pub(crate) fn get_block_hash_by_height(
    &self,
    height: Height,
    rtx: Option<&ReadTransaction>,
  ) -> Result<Option<BlockHash>> {
    if let Some(rtx) = rtx {
      match rtx.open_table(HEIGHT_TO_BLOCK_HASH)?.get(&height.n())? {
        None => Ok(None),
        Some(hash) => Ok(Some(Entry::load(*hash.value()))),
      }
    } else {
      match self
        .database
  ...
  pub(crate) fn get_outpoint_range(&self, outpoint: OutPoint) -> Result<Option<Vec<u8>>> {
    match self
      .database
      .begin_read()?
    ...
    {
      None => Ok(None),
      Some(range) => Ok(Some(range.value().to_vec())),
    }
  }

  pub(crate) fn get_outpoint_address(
    &self,
    outpoint: OutPoint,
    chain: Chain,
    rtx: &ReadTransaction,
    ...
    {
      None => {
        if let Some(tx) = self.get_transaction(outpoint.txid).map_err(|e| {
          Error::msg(format!(
            "txid {} get_transaction failed with error: {}",
            outpoint.txid, e
          ))
        })? {
          match tx.output.get(outpoint.vout as usize) {
            Some(output) => {
              ...
              match Address::from_script(script, chain.network()) {
                Ok(addr) => Ok(addr),
                Err(e) => Err(Error::msg(format!(
                  "outpoint {} address from_script failed with error: {}",
                  outpoint, e
                ))),
              }
            }
            None => Err(Error::msg(format!(
              "outpoint {} vout {} not found in tx {}",
              outpoint, outpoint.vout, outpoint.txid
            ))),
          }
        } else {
          Err(Error::msg(format!(
            "outpoint {} address not found",
            outpoint
          )))
        }
      }
      Some(script_buf) => {
        let script_buf = script_buf.value().to_vec();
        match Address::from_script(
          ScriptBuf::from_bytes(script_buf).as_script(),
          chain.network(),
        ) {
          Ok(addr) => Ok(addr),
          Err(e) => Err(Error::msg(format!(
  ...
  pub(crate) fn get_txid_output_num(
    &self,
    txids: &Vec<Txid>,
    rtx: &ReadTransaction,
  ) -> Result<Vec<u32>> {
    let table = rtx.open_table(TXID_TO_OUTPUT_NUM)?;
    let mut res = vec![];
    for txid in txids {
      match table.get(&txid.store())? {
        None => {
          if let Some(tx) = self.get_transaction(*txid)? {
            res.push(tx.output.len() as u32);
          } else {
            return Err(Error::msg(format!("output_num not found for {}", txid)));
          }
        }
        Some(num) => {
          res.push(num.value());
  ...
  pub(crate) fn get_index_on_blockhash_txid(
    &self,
    blockhash_txid: BlockHashWithTxid,
    rtx: &ReadTransaction,
  ) -> Result<Option<u32>> {
    match rtx
      .open_table(BLOCKHASH_AND_TXID_TO_TXINDEX)?
      .get(&blockhash_txid.store())?
    {
      None => Ok(None),
      Some(idx) => Ok(Some(idx.value())),
    }
  }
}
  ...
  pub(crate) fn open(options: &Options) -> Result<Self> {
      ...
      .open(&path)
    {
      ...
      Err(_) => {
        ...
        tx.open_table(BLOCKHASH_AND_TXID_TO_TXINDEX)?;
        tx.open_table(BLOCK_HASH_TO_HEIGHT)?;
        tx.open_multimap_table(SATPOINT_TO_INSCRIPTION_ID_RECORD)?;
        tx.open_table(OUTPOINT_TO_ADDRESS)?;
        tx.open_table(TXID_TO_OUTPUT_NUM)?;
